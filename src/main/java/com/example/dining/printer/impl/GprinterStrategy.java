...
@Slf4j
@Component
public class GprinterStrategy implements PrinterStrategy {
    private static final String API_URL = "https://open.gprinter.com/api";
    private final RestTemplate restTemplate;

    public GprinterStrategy(RestTemplate restTemplate) {
    ...
    public boolean print(Order order) {
        try {
            ...
            params.put("deviceNo", order.getTableNo()); // 使用桌号作为打印机编号
    ...
    public boolean test(PrinterConfig config) {
        try {
            ...
            // 调用佳博云打印接口
            Map<String, Object> params = new HashMap<>();
            params.put("deviceNo", config.getSn());
            params.put("content", content);
            params.put("copies", 1);
